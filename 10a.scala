...
val first = mutable.HashMap.empty[Int,Int]
val second = mutable.HashMap.empty[Int,Int]
val low = mutable.HashMap.empty[Int,Int]
val high = mutable.HashMap.empty[Int,Int]
val lowType = mutable.HashMap.empty[Int,String]
val highType = mutable.HashMap.empty[Int,String]
val output = mutable.HashMap.empty[Int,Int]
...
def setNextValue(bot:Int, value:Int) {
  if (first.contains(bot)) {
    second.put(bot, value)
  }
  else {
    first.put(bot, value)
  }
...
def pushIfFull(bot:Int) {
  if (first.contains(bot) && second.contains(bot) && low.contains(bot)) {
    printf("Can push bot %s\n", bot)
    if (first(bot) > second(bot)) {
      if (lowType(bot) == "bot") {
        setNextValue(low(bot), second(bot))
        pushIfFull(low(bot))
      }
      else {
        output.put(low(bot), second(bot))
      }
      if (highType(bot) == "bot") {
        setNextValue(high(bot), first(bot))
        pushIfFull(high(bot))
      }
      else {
        output.put(high(bot), first(bot))
      }
    }
    else {
      if (lowType(bot) == "bot") {
        setNextValue(low(bot), first(bot))
        pushIfFull(low(bot))
      }
      else {
        output.put(low(bot), first(bot))
      }
      if (highType(bot) == "bot") {
        setNextValue(high(bot), second(bot))
        pushIfFull(high(bot))
      }
      else {
        output.put(high(bot), second(bot))
      }
    }

  }
  else {
    printf("Unable to push bot %s because %s, %s, %s\n", bot, first.contains(bot), second.contains(bot), low.contains(bot))
  }
}
...
for (item <- input) {
  ...
  if (words(0) == "value") {
    ...
    setNextValue(bot, value)
    pushIfFull(bot)
  }
  else {
    val bot = words(1).toInt
    low.put(bot, words(6).toInt)
    high.put(bot, words(11).toInt)
    lowType.put(bot, words(5))
    highType.put(bot, words(10))
    pushIfFull(bot)
  }
}

print("--- STATE ---")
for ((k,v) <- ListMap(low.toSeq.sortBy(_._1):_*)) {
  printf("bot %s sends low to %s, high to %s", k, v, high(k))
  if (first.contains(k)) {
    printf(", first %s", first(k))
  }
  else {
    printf(", first _")
  }
  if (second.contains(k)) {
    printf(", second %s", second(k))
  }
  else {
    printf(", second _")
  }
...
for ((k,v) <- ListMap(output.toSeq.sortBy(_._1):_*)) {
  printf("output %s is %s\n", k, v)
}
